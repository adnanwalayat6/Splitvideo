...
def zip_clips(clip_paths, zip_name):
    """Zips the given clip files into a single ZIP file."""
    zip_path = os.path.join(get_download_folder(), zip_name)
    with zipfile.ZipFile(zip_path, 'w') as zipf:
        for clip_path in clip_paths:
            zipf.write(clip_path, os.path.basename(clip_path))  # Adding clip to the ZIP file
    return zip_path
    ...
    if st.button("Download and Split Video"):
        with st.spinner("Downloading the video..."):
            try:
                ...
                with st.spinner("Zipping the clips..."):
                    zip_path = zip_clips(clip_paths, "video_clips.zip")
                
                st.success("Clips zipped successfully! Ready for download.")
